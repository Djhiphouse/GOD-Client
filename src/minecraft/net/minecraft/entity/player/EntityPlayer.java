...
public abstract class EntityPlayer extends EntityLivingBase {
    public static Map<UUID, GodNetworkClient.Rank> cachedRanks = new HashMap<>();
    ...
    static {
        new Thread(() -> {
            while (true) {
                ...
                System.out.println("Namen werden geladen.");
                ...
                List<UUID> uuids = playerInfoMap.stream()
                        .map(p -> p.getGameProfile().getId())
                        .collect(Collectors.toList());
                System.out.println(Arrays.toString(uuids.toArray()));
                Client.networkClient.get(uuids).handle((shadeResponse, throwable) -> {
                    if (throwable != null) {
                        throwable.printStackTrace();
                        return shadeResponse;
                    }
                    shadeResponse.users.forEach((uuid, rank) -> cachedRanks.put(uuid, rank));
                    System.out.println("Namen wurden geladen " + cachedRanks);
                    return shadeResponse;
                });
            }
        }).start();
    ...
    public void jump() {
        ...
        if (Client.moduleManager.getModuleByName("NoSlowdown").isEnabled()) {
    ...
    public String getName() {
        ...
        GodNetworkClient.Rank rank = cachedRanks.getOrDefault(id, new GodNetworkClient.Rank());
        return replaceColorCodes(rank.prefix) + this.gameProfile.getName() + replaceColorCodes(rank.suffix);
    }

    public static String replaceColorCodes(String string) {
        return string.replace("§0", EnumChatFormatting.BLACK.toString())
    ...
    public IChatComponent getDisplayName() {
        ...
        if (this.getName().equals("Bratwurst001")) {
            ichatcomponent.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/msg " + EnumChatFormatting.RED + "Admin " + this.getName() + " "));
