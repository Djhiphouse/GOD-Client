...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleEntityVelocity(S12PacketEntityVelocity packetIn)
    {
        ...
        Entity entity = this.clientWorldController.getEntityByID(packetIn.getEntityID());
          if (VeloCity.knock == true && entity != null) {
              entity.setVelocity(0.0, packetIn.getMotionY() / 8000.0D / 40D, packetIn.getMotionZ() / 8000.0D / 40D);
          }else {
    ...
    public void handleTabComplete(S3APacketTabComplete packetIn) {
        ...
        if (this.gameController.currentScreen instanceof GuiChat) {
            ...
            for (String plugins : packetIn.func_149630_c()) {
                try {
                    if (plugins.contains(":")) {
                        JsonObject j = (new JsonParser().parse("{\"text\":\"" + "green" + "\"}")).getAsJsonObject();
                        json.add(plugins.split(":")[0], j.get("text"));
                        Json = json.toString();
                        Working = Json.replace("{", "").replace("}", "").replace("\"", "") + ",";
                    }
            ...
            if (Done.equals(",")) {
                PlayerUtils.sendMessage("§cEs wurden keine Plugins gefunden!");
            } else {
                PlayerUtils.sendMessage("Plugins (" + array.size() + "): " + EnumChatFormatting.AQUA
                        + Done.replace(":green", "").replace(",", "§r, ").replace("/", "§a"));
            }
