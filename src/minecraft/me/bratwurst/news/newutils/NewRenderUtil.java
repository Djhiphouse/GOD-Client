...
public class NewRenderUtil {
    ...
    public static long drawRainbowRectBorder(int left, int top, int right, int bottom, int size) {
        long topCol = NewRenderUtil.renderRainbowRect(left, top - size, right, top, 2.0, 10L, 0L, RainbowDirection.RIGHT);
        long downCol = NewRenderUtil.renderRainbowRect(left - size, top - size, left, bottom + size, 2.0, 10L, 0L, RainbowDirection.DOWN);
        NewRenderUtil.renderRainbowRect(right, top - size, right + size, bottom + size, 2.0, 10L, topCol, RainbowDirection.DOWN);
        NewRenderUtil.renderRainbowRect(left, bottom, right, bottom + size, 2.0, 10L, downCol, RainbowDirection.RIGHT);
        return topCol;
    }
        ...
        switch (rainbowDirection) {
            case RIGHT: {
                for (int i = 0; i < right - left; ++i) {
                    ...
                    Gui.drawRect(left + i, top, right, bottom, NewColors.getRainbow(endDelay, time).getRGB());
                }
                break;
            }
            case LEFT: {
            ...
            case DOWN: {
                for (int i = 0; i < bottom - top; ++i) {
                    endDelay = delay + (long)i * -difference;
                    Gui.drawRect(left, top + i, right, bottom, NewColors.getRainbow(endDelay, time).getRGB());
                }
                break;
            }
            case UP: {
    ...
    public static long drawLine(int fromX, int fromY, int toX, int toY, int steps, long startOffset) {
        ...
        while ((double)i2 < count) {
            ...
            Color rgb = NewRenderUtil.rainbowColor(offset, 1.0f);
