...
//	static ArrayList<Vec3> positionsBack = new ArrayList<Vec3>();
        ...
        for (int i = 0; i < steps; i++) {
            ...
            if (i != steps - 1) {
                ...
                if (Math.abs(yDist) < maxYTP && mc.thePlayer.getDistance(posX, posY, posZ) >= 4) {
                    ...
                    mc.getNetHandler().getNetworkManager().sendPacket(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.START_DESTROY_BLOCK, targetBlockPos, EnumFacing.UP));
                    mc.getNetHandler().getNetworkManager().sendPacket(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK, targetBlockPos, EnumFacing.UP));
                } else {
                    attack = false;
                }
            }
    ...
    private static void attackInf(EntityLivingBase en) {
        ...
        if ((Client.getInstance().getModuleManager().getModuleByName("Criticals").isEnabled())) {
